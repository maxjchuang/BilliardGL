...
PACKAGE = Path(__file__).parent / "reference_data/pocket_geometry_analytic_contract"


class PocketGeometryAnalyticAdapterTests(unittest.TestCase):
    def test_grade_c_package_has_immutable_balanced_partitions(self):
        ...
        self.assertTrue(all(json.loads(case.scenario_json)["schema_version"] == 7
                            for case in adaptation))

    def test_holdout_values_cannot_change_calibration_scenarios(self):
        package = load_reference_package(PACKAGE)
        points = read_reference_points(package.files["normalized"], package.manifest["dataset_id"])
        split = load_reference_split(package.files["split"], points,
                                     package.manifest["dataset_id"],
